...
class TopicAnalysisSchema(BaseModel):
    ...
    intent: Literal['answer', 'clarification', 'new_topic', 'off_topic_question'] = Field(
        ...,
        description="User's intent: 'answer' (answering lesson question), 'clarification' (asking about current topic), 'new_topic' (wants to learn something new), 'off_topic_question' (unrelated question)"
    )
    ...
    suggested_action: Literal['continue_lesson', 'answer_and_continue', 'switch_topic', 'politely_redirect'] = Field(
        ...,
        description="Suggested action: 'continue_lesson' (proceed with current lesson), 'answer_and_continue' (answer question then continue), 'switch_topic' (start new lesson), 'politely_redirect' (redirect to current lesson)"
    )